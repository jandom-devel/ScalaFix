...
object Assignment:
  ...
  /**
   * An assignment which reproduces the value of a map. Although the map might
   * be directly used as an assignment, this class overrides the `toString`
   * method to be more useful for debugging.
   *
   * @param m
   *   the map
   */
  ...
  /**
   * An assignment obtained from another assignment by modifying the value of an
   * unknown with a given value.
   *
   * @param special
  ...
  private final class ConditionalAssignment[-U, +V](
      special: U,
      value: V,
      otherwise: Assignment[U, V]
  ) extends Assignment[U, V]:
