...
)(using val dom: Domain[V]) extends EquationSystemBase[U, V] with GraphEquationSystem[U, V, E]:
  ...
  val body: Body[U, V] =
    (rho: Assignment[U, V]) =>
      (x: U) =>
        ...
        val res = if contributions.isEmpty then
          rho(x)
        else
          contributions reduce (_  upperBound _)
      ...
      (rho: Assignment[U, V]) =>
        (e: E) =>
          ...
          if boxes.isDefinedAt(x) && sources(e).exists(ordering.lteq(x, _)) then
            ...
            edgeAction(rho)(e)
    if boxes.boxesAreIdempotent then
        ...
        (x: U) =>
          ...
          if contributions.isEmpty then
            ...
            val result = contributions reduce {
              (x: (V, Boolean), y: (V, Boolean)) => (x._1 upperBound y._1, x._2 || y._2)
            }
            if result._2 then
