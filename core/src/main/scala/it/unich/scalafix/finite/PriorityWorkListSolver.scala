...
object PriorityWorkListSolver:
   ...
   * @param ordering
   *   an ordering which specifies priorities between unknowns (defaults to the
   *   depth-first ordering over `eqs`)
   * @param restart
   *   at each iteration this function is applied to the new and old values. If
   *   it returns true, the analysis of bigger unknown is restarted from the
   *   initial value. (defaults to constant `false`)
   * @param tracer
