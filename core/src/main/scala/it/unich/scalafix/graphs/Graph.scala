...
/**
 * A body obtained from an hyper-graph. In ScalaFix, an hyper-graph a graph
 * where each edge as several source nodes and a single target node. Unknowns
 ...
 *
 * @tparam U
 *   the type for the unknowns of this body, which are also the nodes of the
 *   graph.
 * @tparam V
 *   the type for the values assumed by the unknowns of this body, which is also
 *   the type of the values generated by each edge trough the `edgeAction`.
 * @tparam E
 *   type of edges.
 * @param sources
...
case class Graph[U, V, E](
  ...
  /**
   * Returns a new graph obtained by adding combos to the rhs.
   *
   * @param combos
  ...
  /**
   * Add warrowings to the equation system in a localized way. Localized
   * warrowings require a different procedure than standard localized widenings.
