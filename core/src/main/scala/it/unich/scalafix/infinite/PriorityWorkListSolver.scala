...
object PriorityWorkListSolver:
  ...
  /**
   * This is an dynamic ordering on unknowns: every time an unknown appears, it
   * gets assigned a lower priority than previous one (i.e., it comes earlier in
   * the ordering). This is the default ordering for PriorityWorkListSolver when
   * an explicit one is not provided.
   */
  class DynamicPriority[U] extends Ordering[U]:
    ...
    def compare(x: U, y: U): Int =
      val xp = map.getOrElseUpdate(
        x, {
          current -= 1
          current
        }
      )
      val yp = map.getOrElseUpdate(
        y, {
          current -= 1
          current
        }
   ...
   * @param ordering
   *   an ordering which specifies priorities between unknowns (defaults to a
   *   dynamical ordering induced by evaluation)
   * @param tracer
