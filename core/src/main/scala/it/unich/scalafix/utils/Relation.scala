...
/**
 * This trait represents an arbitrary mathematical relation over values of type
 * `A`. It extends `A => collection.Set[A]` and the apply method is guaranteed
 * to return a set (eventually empty) for each element of type `A`.
 *
 * @tparam A
...
object Relation:
  ...
  private class InfluenceRelationFromHash[A](hash: collection.Map[A, collection.Set[A]])
      extends Relation[A]:
    def apply(x: A): collection.Set[A] = hash.getOrElse(x, Set.empty[A])

  /**
   * Returns an influence relation given a function `A => Set[A]`. To respect
   * the contract of a `Relation`, the map `f` should return a set for every
   * possible value of `A`-
   */
  ...
  /**
   * Returns an relation given a traversable collection of pairs `(u -> v)`,
   * each pair meaning that `u` relates to `v`. When iterating over the image of
  ...
  def apply[A](graph: Seq[(A, A)]): Relation[A] =
    ...
    for (u, v) <- graph do hash.getOrElseUpdate(u, mutable.LinkedHashSet.empty[A]) += v
