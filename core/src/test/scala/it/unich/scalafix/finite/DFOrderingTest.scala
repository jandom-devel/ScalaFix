...
class DFOrderingTest extends AnyFunSpec with ScalaCheckPropertyChecks {
  ...
  private val graph = Relation(Seq(1 -> 3, 1 -> 2, 2 -> 3, 3 -> 4, 4 -> 6, 4 -> 5, 4 -> 8, 5 -> 7, 6 -> 7, 7 -> 4,
    7 -> 8, 8 -> 4, 8 -> 3, 8 -> 10, 8 -> 9, 9 -> 1, 10 -> 7))
  private val heads = Set(1, 3, 4, 7)
  private val sequence = 1 to 10
  private val advancing = Set(1 -> 2, 1 -> 3, 3 -> 4, 4 -> 5, 4 -> 6, 6 -> 7, 7 -> 8, 8 -> 9, 8 -> 10, 4 -> 8)
  private val retreating = Set(10 -> 7, 9 -> 1, 8 -> 3, 7 -> 4, 4 -> 3)
  private val cross = Set(2 -> 3, 5 -> 7)

  private val graph2 = Relation(Seq(1 -> 2, 1 -> 3, 2 -> 3, 3 -> 4, 4 -> 6, 4 -> 5, 4 -> 8, 5 -> 7, 6 -> 7, 7 -> 4,
    7 -> 8, 8 -> 4, 8 -> 3, 8 -> 9, 8 -> 10, 9 -> 1, 10 -> 7))
