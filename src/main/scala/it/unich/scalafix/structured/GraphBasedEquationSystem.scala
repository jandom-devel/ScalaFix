...
/**
  ...
  * an equation system by itself, and may be though of as an hyperedge with many sources and a single
  * target.
  *
...
abstract class GraphBasedEquationSystem[U, V: Magma, E] extends FiniteEquationSystem[U, V] {
  ...
  /**
    * A relation between edges and unknowns. The pair (e,u) is in the relation if u is a sources
  ...
  /**
    ...
    */
  def withLocalizedBoxes(boxes: BoxAssignment[U, V], ordering: Ordering[U]): GraphBasedEquationSystem[U, V, _]
...
object GraphBasedEquationSystem {
  ...
  final case class SimpleGraphBasedEquationSystem[U, V, E](
    ...
    val infl: InfluenceRelation[U] = new InfluenceRelation[U] with InfluenceRelation.WithDiagonal[U] {
      val graph = for (x <- unknowns; e <- ingoing.image(x); y <- sources.image(e)) yield (y, x)

      def apply(u: U) = for (e <- outgoing.image(u); y <- targets.image(e)) yield y
    }
    ...
    def withLocalizedBoxes(boxes: BoxAssignment[U, V], ordering: Ordering[U]) = {
      val newEdgeBody: EdgeBody[U, V, E] = { e: E =>
        (rho: U => V) => x: U =>
          if (boxes.isDefinedAt(x) && sources.image(e).exists {
            ordering.lteq(x, _)
          }) {
      ...
      val newSources = if (boxes.areIdempotent)
        sources
      else {
        val newgraph: Set[(E, U)] = (for (x <- unknowns; e <- ingoing.image(x); if sources.image(e) exists {
          ordering.lteq(x, _)
        })
