...
/**
  ...
  * @tparam V the co-domain of the relation.
  */
trait Relation[U, V] extends PartiallyOrdered[Relation[U, V]] {
  /**
    * Returns whether `u` is in relation with `v`.
    */
  ...
  /**
    * The same relation with the addition of the identity relation
    */
  def withDiagonal(implicit ev: U=:=V): Relation[U,V] = this union Relation(this.domain, { (u:U) => Set(u) })
}

/**
  * The companion object of Relation, with factory methods.
  */
object Relation {

  /**
    * This is a trait which may be mixed in with a Relation in order to automatically
  ...
  private final class RelationFromImageMap[U, V](map: Map[U, Set[V]]) extends Relation[U, V] with AutomaticInverse[U, V] with AutomaticPartialOrdering[U, V] {
    def isEmpty = map.forall { (kv: (U, Set[V])) => kv._2.isEmpty }
    ...
    lazy val codomain = inverse.domain
  }

  /**
    * Builds a relation from U to V given a map from U to Set[V].
    */
  def apply[U, V](map: Map[U, Set[V]]): Relation[U, V] = new RelationFromImageMap(map)
  ...
  def apply[U, V](dom: Set[U], f: U => Set[V]): Relation[U, V] = new Relation[U, V] with AutomaticInverse[U, V] with AutomaticPartialOrdering[U, V] {
    def isEmpty = dom.forall(x => f(x).isEmpty)
