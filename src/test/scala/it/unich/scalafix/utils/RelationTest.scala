...
class RelationTest extends FunSuite with PropertyChecks {
  ...
  def testMap(r: Relation[Int, Int], m: Map[Int, Set[Int]]) {
    assert(r == r)
  ...
  test("Non empty relations generated by sequences") {
    forAll { (m: Map[Int, Set[Int]]) =>
      testMap(Relation(m), m)
      val graph: Seq[(Int, Int)] = (for (u <- m.keySet; v <- m(u)) yield (u, v))(collection.breakOut)
      testMap(Relation(graph: _*), m)
    }
  }
